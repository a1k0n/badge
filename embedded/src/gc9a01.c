...
static void gc9a01_gpio_init(gc9a01_t *display) {
    ...
    gpio_put(cfg->pin_rst, 1); // RST high (not in reset)
}

static void gc9a01_spi_init(gc9a01_t *display) {
    spi_init(display->config.spi_port, display->config.baudrate);
    spi_set_format(display->config.spi_port, 8, SPI_CPOL_0, SPI_CPHA_0, SPI_MSB_FIRST);
}
...
static void gc9a01_dma_init(gc9a01_t *display) {
    ...
    dma_channel_config c = dma_channel_get_default_config(display->config.dma_channel);
    channel_config_set_transfer_data_size(&c, DMA_SIZE_8);
    channel_config_set_dreq(&c, spi_get_dreq(display->config.spi_port, true));
    
    dma_channel_configure(
        display->config.dma_channel,
        &c,
...
void gc9a01_reset(gc9a01_t *display) {
    gpio_put(display->config.pin_rst, 0);
    sleep_ms(10);
    gpio_put(display->config.pin_rst, 1);
    sleep_ms(10);
}
...
void gc9a01_fill_screen(gc9a01_t *display, uint16_t color) {
    ...
    for (int y = 0; y < GC9A01_HEIGHT; y++) {
        gc9a01_write_data(display, (const uint8_t*)fill_buffer, sizeof(fill_buffer));
    }
} 